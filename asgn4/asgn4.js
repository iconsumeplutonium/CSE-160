...
var FSHADER_SOURCE = `
    ...
    void main() { //TODO: add camera script from 3b, use camera.eye for v_cameraPos
        vec3 dirToLight = normalize(u_lightPos - v_Position.xyz);
        vec3 dirToCamera = normalize(v_cameraPos - v_Position.xyz);
        vec3 reflectionVector = normalize(reflect(dirToLight, v_Normal));

        float ambient = ambientConstant;
        float diffuse = dot(dirToLight, v_Normal) * diffuseConstant;
        float specular = pow(dot(reflectionVector, v_cameraPos), alpha) * specularConstant;

        gl_FragColor = (ambient + diffuse + specular) * vec4(0.0, 0.0, 1.0, 1.0); //v_Color;
    }`;


let canvas, gl;
let a_Position, a_Color, u_ModelMatrix, u_GlobalRotationMatrix, a_Normal, u_NormalMatrix, a_cameraPos, u_lightPos;
let specularConstant, diffuseConstant, ambientConstant, alpha;
...
function connectVariablesToGLSL() {
    ...
    diffuseConstant = gl.getUniformLocation(gl.program, 'diffuseConstant');
    if (!diffuseConstant) {
        console.log('Failed to get the storage location of diffuseConstant');
        return;
    }

    specularConstant = gl.getUniformLocation(gl.program, 'specularConstant');
    if (!specularConstant) {
        console.log('Failed to get the storage location of specularConstant');
        return;
    }

    ambientConstant = gl.getUniformLocation(gl.program, 'ambientConstant');
    if (!ambientConstant) {
        console.log('Failed to get the storage location of ambientConstant');
        return;
    }

    alpha = gl.getUniformLocation(gl.program, 'alpha');
    if (!alpha) {
        console.log('Failed to get the storage location of alpha');
        return;
    }

    a_cameraPos  = gl.getAttribLocation(gl.program, 'a_cameraPos');
    if (!a_cameraPos) {
...
function main() {
    ...
    gl.uniform1f(specularConstant, specular);
    gl.uniform1f(diffuseConstant, diffuse);
    gl.uniform1f(ambientConstant, ambient);
    gl.uniform1f(alpha, alpha1);

    gl.uniform3fv(u_lightPos, new Float32Array(headRotation));
...
function renderAllShapes(useSliderValues = true) {
    ...
    gl.uniform3fv(u_lightPos, new Float32Array(leftArmRotation));

    // Assuming xSlider, ySlider, and zSlider are your sliders
    let x = xSlider.value; // Convert to radians
    let y = ySlider.value;
    let z = zSlider.value;

    // Create rotation matrices
    let Rx = new Matrix4().setRotate(x, 1, 0, 0);
    let Ry = new Matrix4().setRotate(y, 0, 1, 0);
    let Rz = new Matrix4().setRotate(z, 0, 0, 1);

    // Apply rotations
    glob.setIdentity(); // Reset to identity matrix before applying rotations
    glob.concat(Rz);
    glob.concat(Ry);
    glob.concat(Rx);

    // Calculate camera position in world space
    let globInverse = new Matrix4().setInverseOf(glob);
    let cameraPosition = globInverse.multiplyVector3(new Vector3([0, 0, 1])); // The camera's local position is at the origin

    console.log(cameraPosition.elements);

    let cameraPosBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, cameraPosBuffer);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(cameraPosition.elements), gl.DYNAMIC_DRAW);
    gl.vertexAttribPointer(a_Position, 3, gl.FLOAT, false, 0, 0);
    //gl.enableVertexAttribArray(a_cameraPos);

    let sphere = new Sphere(15, new Matrix4(), 0.25);
    sphere.render();
